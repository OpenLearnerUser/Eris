...
namespace Eris
{
    ...
    T MatrixCsrVectorMul<T, VE>::operator[](size_t i) const
    {
        ...
        auto nnz = _m.nonZeroBegin();      // 非零值数组 (长度 nnz)
        ...
        const std::initializer_list<std::initializer_list<T>> &lst, T epsilon)
    {
        ...
        {
            ERIS_ASSERT(rowIter->size() == numCols);
    ...
    {
        ERIS_ASSERT(nonZeros.size() == columnIndices.size());
        ++_size.x;
