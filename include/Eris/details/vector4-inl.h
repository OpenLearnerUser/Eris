...
namespace Eris
{
    ...
    {
        return Eris::absmin(Eris::absmin(x, y), Eris::absmin(z, w));
    }

    template <typename T>
    T Vector<T, 4>::absmax() const
    {
        return Eris::absmax(Eris::absmax(x, y), Eris::absmax(z, w));
    }
                                     ...
                                     T f)
    {
        ...
        if (std::fabs(D1.w) < std::numeric_limits<T>::epsilon() ||
