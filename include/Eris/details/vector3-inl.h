...
namespace Eris
{
    ...
    template <typename T>
    void Vector<T, 3>::set(T v)
    {
        x = y = z = v;
    }

    template <typename T>
    void Vector<T, 3>::set(T x_, T y_, T z_)
    {
        x = x_;
        y = y_;
        z = z_;
    ...
    template <typename T>
    void Vector<T, 3>::set(const Vector &pt)
    {
        x = pt.x;
        y = pt.y;
        z = pt.z;
    ...
    {
        return std::min(x, std::min(z, y));
    }

    template <typename T>
    T Vector<T, 3>::max() const
    {
        return std::max(x, std::max(z, y));
    }

    template <typename T>
    T Vector<T, 3>::absmin() const
    {
        return Eris::absmin(x, Eris::absmin(y, z));
    }

    template <typename T>
    T Vector<T, 3>::absmax() const
    {
        return Eris::absmax(x, Eris::absmax(y, z));
    }
    ...
    template <typename T>
    bool Vector<T, 3>::isSimilar(const Vector &other,
                                 T epsilon) const
    {
    ...
    //! Returns element-wise min vector: (min(a.x, b.x), min(a.y, b.y), min(a.z, b.z)).
    template <typename T>
    ...
    //! Returns element-wise max vector: (max(a.x, b.x), max(a.y, b.y), max(a.z, b.z)).
    template <typename T>
                                     ...
                                     T f)
    {
        ...
        if (std::fabs(D1.z) < std::numeric_limits<T>::epsilon() ||
