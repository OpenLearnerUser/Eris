...
namespace Eris
{
    ...
    void parallelFor(Indextype start, Indextype end,
                     const Function &func, ExecutionPolicy policy)
    {
        if (start >= end)
            return;
        ...
        unsigned int numThreads = Eris::maxNumberOfThreads();
        if (numThreads == 0)
            numThreads = 8;

        Indextype n = end - start;
        // 限制线程数不超过任务数
        numThreads = std::min<unsigned int>(numThreads, n);
        ...
        // 计算每个线程的任务量
        Indextype slice = n / numThreads;
        Indextype remainder = n % numThreads;
        ...
        {
            Indextype taskSize = slice + (i < remainder ? 1 : 0);
            Indextype next = current + taskSize;

            // 重要：按值捕获 func，避免引用失效
            pool.emplace_back([&func, current, next]()
                              {
            for (auto k = current; k < next; ++k) {
                func(k);
            } });
            current = next;
        }
                          ...
                          const Function &function, ExecutionPolicy policy)
    {
        parallelRangeFor(beginIndexY, endIndexY, [&](Indextype jBegin, Indextype jEnd)
                         { function(beginIndexX, endIndexX, jBegin, jEnd); }, policy);
    }
